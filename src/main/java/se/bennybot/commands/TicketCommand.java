    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        if (event.getName().equals("setup-ticket")) {
            ...
            if (event.getMember().hasPermission(Permission.ADMINISTRATOR)) {
                ...
                role = event.getOption("access_role").getAsRole();
                String title = event.getOption("title").getAsString();
                String content = event.getOption("description").getAsString();
                String button_text = event.getOption("button_text").getAsString();
                boolean isBox = event.getOption("helpbox").getAsBoolean();

                EmbedBuilder embed = EmbedBuilderTemplate.simpleEmbedBuilderTemplate(title, event.getGuild().getName(), content, "", "", "", "");
                Button button = Button.success("open", button_text).withEmoji(Emoji.fromFormatted("\uD83C\uDF9F\uFE0F"));

                if (isBox) {
                    embed.addField("How to use", "Click on that button below and a ticket will open!", true);
                }
                EmbedBuilder success_setup_embed = EmbedBuilderTemplate.simpleEmbedBuilderTemplate("Success :white_check_mark:", event.getGuild().getName(),"Successfully setuped Ticket-System!","", "", "", "");
                event.replyEmbeds(success_setup_embed.build()).setEphemeral(true).queue();
